...
def get_hexagram_info(i):
    """
    根据 make64Table-Fix-v4.py 的逻辑解析卦象
    """
...
def generate_score_txt(output_path):
    ...
    lines.append("  - 八卦五行对应源自《说卦传》及汉代象数易学")
    lines.append("  - 五音五行对应源自《黄帝内经》《礼记·月令》")
    lines.append("  - 高八度规则为创作者艺术设计")
    lines.append("")
...
def main():
    print("=" * 60)
    print("  伏羲六十四卦 - 简谱与MIDI生成器 v4")
    print("  (基于 make64Table-Fix-v4.py 修正版)")
    print("=" * 60)
    print()
    ...
    # 生成简谱TXT
    txt_path = os.path.join(OUTPUT_DIR, "YiJing_64_Score_v4.txt")
    generate_score_txt(txt_path)
    print(f"  简谱TXT: {txt_path}")
    
    # 生成MIDI
    if HAS_MIDI:
        midi_path = os.path.join(OUTPUT_DIR, "YiJing_64_v4.mid")
